...
export default function TableViewV2({ recommendations, metrics }: Props) {
  ...
  const closeLocTimer = useRef<ReturnType<typeof setTimeout> | null>(null)

  const scheduleLocClose = () => {
    closeLocTimer.current = setTimeout(() => setShowLocHover(false), 120)
  }

  const cancelLocClose = () => {
    if (closeLocTimer.current) clearTimeout(closeLocTimer.current)
  }
                          ...
                          {rec.effort === 'Quick win' && (
                            <Tooltip>
                              <TooltipTrigger asChild>
                                <img src={`${BASE}assets/electric_bolt.svg`} alt="Quick win" className="w-4 h-4 cursor-default" />
                              </TooltipTrigger>
                              <TooltipContent side="bottom">Quick win</TooltipContent>
                            </Tooltip>
                          )}
                          {rec.effort === 'Bigger lift' && (
                            <Tooltip>
                              <TooltipTrigger asChild>
                                <img src={`${BASE}assets/lead.svg`} alt="High impact" className="w-4 h-4 cursor-default" />
                              </TooltipTrigger>
                              <TooltipContent side="bottom">High impact</TooltipContent>
                            </Tooltip>
                          )}
                        </span>
                    ...
                    <TableCell className="py-4 align-top">
                      ...
                      <div className="flex items-center gap-1 min-h-[36px]">
                        ...
                        <button
                          ...
                          className={`flex items-center justify-center w-[30px] h-[30px] hover:bg-[#eaeaea] rounded-none transition-colors flex-shrink-0 transition-opacity duration-200 ${isHovered ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}
                          onClick={e => { e.stopPropagation(); handleChevronEnter(rec) }}
                          onMouseEnter={e => { e.stopPropagation(); handleChevronEnter(rec) }}
                          onMouseLeave={() => scheduleLocClose()}
                        >
                          <DownChevronIcon />
                        </button>
                      </div>

                      {/* Reject + Accept — absolutely positioned at row's right edge */}
                      <div
                        className={`absolute right-4 top-4 flex items-center gap-1 transition-opacity duration-200 ${isHovered ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}
                        ...
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <Button
                              ...
                              className="w-[30px] h-[30px] rounded-none border-[#eaeaea] text-[#555] hover:text-[#212121] hover:bg-[#f5f5f5] flex-shrink-0"
                              onClick={e => e.stopPropagation()}
                            >
